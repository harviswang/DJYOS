...
/*____与硬件相关的配置____*/
#define M 1000000
#define cn_mclk (72*M)  //主频
#define cn_fclk cn_mclk  //cm3自由运行外设时钟
#define cn_ahbclk cn_mclk  //总线时钟
#define cn_pclk2 cn_mclk  //高速外设时钟
#define cn_pclk1 (cn_mclk/2)  //低速外设时钟
#define cn_extclk 8000000  //外部时钟=8M

/*____以下定义tick参数____*/
#define cn_tick_us 1000  //操作系统内核时钟脉冲长度，以us为单位。
#define cn_tick_hz 1000  //内核时钟频率，单位为hz。
#define cn_fine_us 0x0000038e  //1/72M,精密时钟脉冲长度，以uS为单位，32位定点数整数、小数各占16位，这也限制了ticks最长不超过65535uS
#define cn_fine_hz cn_mclk  //内核精密时钟频率，是cn_fine_ns的倒数。

/*____时间相关设置____*/
#define cfg_time_module 1  //是否包含时间模块
#define cfg_time_zone (+8)  //本地时区
#define cn_time_src_tick 0  //内核嘀嗒脉冲做走时基准
#define cn_time_src_rtc 1  //硬件rtc做走时基准
#define cfg_time_src cn_time_src_rtc  //选择走时基准
...
/*____数量控制____*/
#define cn_device_limit 10  //定义设备数量。
#define cn_locks_limit 20  //定义程序中可用的自由锁的数量。自由锁指用户调用semp_create和mutex_create创建的锁，不包括内核用到的锁。
#define cn_events_limit 20  //事件数量
#define cn_evtts_limit 20  //总事件类型数
#define cn_paras_limit (cn_events_limit+cn_evtts_limit)  //事件参数数量，
#define cn_wdts_limit 1  //允许养狗数量
#define cn_pipes_limit 1  //管道数量

/*____与内存管理相关的常量____*/
#define cn_virtual_memory 0  //m3硬件不支持虚拟内存，如果在硬件支持虚拟内存的机器上，软件不打算支持，也设为0
#define cn_phy_memoey 0  //物理内存总量，支持虚拟内存才有意义。
#define cn_page_size 0x400  //基页尺寸为1Kbytes
#define cn_page_size_suffix_zero 10  //页尺寸后缀0的个数
#define cn_mem_recycle true  //支持内存回收----配置工具
#define cn_mem_pools 5  //允许建立5个内存池
#define cn_syssvc_stack 1024  //系统服务事件栈容量
...
#define cn_opened_fsnode_limit 16  //可同时打开的文件数量，注意多级路径的每一级都算一个文件，但相同的路径不重复算
