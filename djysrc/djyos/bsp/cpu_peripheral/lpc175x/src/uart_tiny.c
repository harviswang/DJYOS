...
bool_t uart_set_baud(u32 uart_no,u32 baud)
{
    ...
    temp = cn_pclk/16/baud;
    temp1 = cn_pclk/16%baud;
    ...
    //如果误差 < cn_pclk/16>>6 = 1/64，说明不用小数波特率误差已经很小。
    //第二个条件是17xx系列uart的要求，分配系数大于2才能用小数波特率
    if((err <= (cn_pclk/16>>6)) || (temp <= 2))
    {
    ...
    for(divval = 1; divval < 15; divval++)
    {
        ...
        {
            temp1 = cn_pclk * mulval / 16 / (mulval + divval) % baud;
            if(temp1 > (baud >> 1))
            {
                err = baud - temp1;
            }else
    ...
    if(result == true)
    {
        temp = (cn_pclk * mulval / 16 / (mulval + divval) + baud -1) / baud;
        reg->LCR |= 1<<bo_uart_lcr_dlab;
        reg->RTD.DLL = temp;
        reg->DI.DLM = temp>>8;
